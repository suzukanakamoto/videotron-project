...
/* ────────────────────────────
   7. IMAGE MODAL (SHOW/HIDE FULL IMAGE)
   ──────────────────────────── */
(function initImageModal() {
  const modal = document.getElementById('imageModal');
  const fullImage = document.getElementById('fullImage');
  const closeModal = document.getElementById('closeModal');
  const cardImages = document.querySelectorAll('.card-img img'); // Menargetkan semua gambar produk

  if (!modal || !fullImage || !closeModal) return;

  // Fungsi untuk membuka modal saat gambar di klik
  cardImages.forEach(img => {
    img.addEventListener('click', function() {
      fullImage.src = this.src; // Mengambil attribute 'src' dari gambar yang diklik
      modal.classList.add('show');
      document.body.style.overflow = 'hidden'; // Mencegah background agar tidak bisa di-scroll
    });
  });
  ...
  const hideModal = () => {
    ...
    document.body.style.overflow = ''; // Mengembalikan fungsi scroll pada background
    
    // Menghapus src setelah transisi selesai agar gambar sebelumnya tidak terlihat sekilas
    setTimeout(() => {
  ...
  // Menutup modal jika area gelap (background) diklik
  ...
  // Menutup modal jika tombol 'Escape' pada keyboard ditekan
